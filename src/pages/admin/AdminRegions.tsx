...
const AdminRegions = () => {
  ...
  const refresh = async () => {
    const data = await apiGetRegions();
    setRegionsList(data);
  };
  ...
  const handleAdd = async () => {
    ...
    const exists = regionsList.some(r => r.toLowerCase() === trimmed.toLowerCase());
    if (exists) {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex gap-3 mb-2">
        <input
          type="text"
          value={newRegion}
          onChange={(e) => { setNewRegion(e.target.value); setError(''); }}
      ...
      <div className="bg-card rounded-2xl card-shadow overflow-hidden mt-4">
        {regionsList.length === 0 ? (
          <p className="text-center text-muted-foreground text-sm py-8">Nenhum estado cadastrado.</p>
        ) : (
...
export default function AdminRegionsWrapper() {
  return (
    <ErrorBoundary>
      <AdminRegions />
    </ErrorBoundary>
